...
//
// Field → real API mapping (all mocked here, documented for the production swap):
...
// Back-compat alias: the verify screen only reads `verified`/`carrier`.
export type VerificationResult = SecurityProfile;
...
export class MockSnaProvider implements IdentityProvider {
  ...
  async confirmAction(purpose: string, amountKzt?: number): Promise<StepUpResult> {
    await delay(700); // carrier-side step-up re-auth
    // a real step-up token is scoped to the action it authorizes (purpose + amount)
    const scope = `${purpose}:${amountKzt ?? 0}`;
    return { confirmed: true, token: `act_${scope.length.toString(36)}${Math.round(performance.now()).toString(36)}` };
  }
}
